...
int fmc_imageon_enable( camera_config_t *config )
{
   ...
   xil_printf( "Output static Frame buffer for 1 second...\n\r" );
   sleep(1);
...
//   fmc_imageon_enable_tpg(config);
   ...
   xil_printf( "Output Video input source in Hardware mode for 2 seconds\n\r" );
   sleep(2);
...
int fmc_imageon_enable_ipipe( camera_config_t *config ) {
   ...
   // Hint 2: You will need to dig through some header files some of the values,
   // Hint 3: Set up the Demosaic IP block first. It is a good warm up.
   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x020, 0x1); //Input Video Format, pg29 4:4:4 = 1
   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x028, 0x2); //Output Video Format, pg29 4:2:2 = 2
   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x18, 0x438); // video height: 1920
   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x10, 0x780); //video width: 1080
//   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x010, 0x1); //Input Video Format, pg29 4:4:4 = 1
//   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x018, 0x2); //Output Video Format, pg29 4:2:2 = 2
//   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x028, 0x780); // video height: 1920
//   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR) + 0x020, 0x438); //video width: 1080

   Xil_Out32((XPAR_XVPROCSS_1_BASEADDR  ) + (XV_HCRESAMPLER_CTRL_ADDR_AP_CTRL), (u32)(0x81));  // Control
   ...
   // For this IP core a High level API was for setting it up.  Trace through these calls to see how much work they do.
   ...
   // config DEMOSAICING
   Xil_Out32((XPAR_XV_DEMOSAIC_0_S_AXI_CTRL_BASEADDR) + 0x0010, 0x780 ); //Active_Width Reg
   Xil_Out32((XPAR_XV_DEMOSAIC_0_S_AXI_CTRL_BASEADDR) + 0x0018, 0x438 ); //Active_Height Reg
   Xil_Out32((XPAR_XV_DEMOSAIC_0_S_AXI_CTRL_BASEADDR) + 0x0028, 0x0 ); //Bayer_Phase Reg
